...
L_corner_size = [L_corner_inner_size[X] + L_corner_wall_th,
                 ...
                 L_corner_inner_size[Z] + L_corner_bottom_th ];

//-- Outer corner radius                 
L_cr = 3;
L_inner_cr = 12;  //-- Inner corner radius
...
{
  difference() {
    bcube([L_corner_size[X], L_corner_size[Y], 4], cr = L_cr, cres = 4);

    //--- Left drill
  translate([cap_drill_pos1[X], cap_drill_pos1[Y], 0])
    cylinder(r = M3_screw_diam/2, h = L_corner_size[Z], center = true);
  
  translate([-cap_drill_pos2[X], -cap_drill_pos2[Y],  0 ])
      cylinder(r = M3_screw_diam/2, h = L_corner_size[Z], center = true);  
  }
}
